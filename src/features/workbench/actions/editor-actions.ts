...
export const EDITOR_ACTIONS: readonly EditorActionDef[] = [
  { id: "editor.find", title: "Find in File", category: "Navigate", keys: ["Mod+F"], command: openSearchPanel },
...
export const buildEditorKeymapExtension = (overrides: Record<string, string[]>): Extension => {
  ...
  for (const def of EDITOR_ACTIONS) {
    ...
    for (const spec of keys) {
      bindings.push({ key: specToCmKey(spec), run: def.command, preventDefault: true });
    }
  }

  return keymap.of([...bindings, indentWithTab, ...standardKeymap, ...historyKeymap]);
};
