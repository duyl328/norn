...
describe("StatusBar", () => {
  ...
  it("编码切换菜单使用状态栏专属毛玻璃样式", async () => {
    ...
    expect(await screen.findByText("All encodings")).toBeInTheDocument();
  ...
  it("显示并切换换行符格式", async () => {
    ...
    expect(await screen.findByText("Line ending")).toBeInTheDocument();
  ...
  it("点击光标位置可输入行号并跳转", () => {
    ...
    const input = screen.getByRole("textbox", { name: "Go to line" });
  ...
  it("快捷键请求会打开醒目的行号输入框", async () => {
    ...
    const input = await screen.findByRole("textbox", { name: "Go to line" });
  ...
  it("按 Esc 关闭跳转输入并请求恢复编辑器焦点", async () => {
    ...
    expect(screen.queryByRole("textbox", { name: "Go to line" })).not.toBeInTheDocument();
  });
  ...
  it("快捷键请求 id 变化才打开输入框", async () => {
    ...
    expect(await screen.findByRole("textbox", { name: "Go to line" })).toHaveValue("2,1");
  });
